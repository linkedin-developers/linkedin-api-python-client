...
from linkedin_api.common.constants import NON_VERSIONED_BASE_URL, RESTLI_METHOD_TO_HTTP_METHOD_MAP, RESTLI_METHODS, VERSIONED_BASE_URL, HTTP_METHODS
    ...
    [
        ...
        (
            ...
            },
            {
                ...
                "path": "/testResource",
                "checked_headers": {
                    "X-RestLi-Method": RESTLI_METHODS.GET_ALL.value
                },
                "response_properties": {
                    "elements": [{"name": "A"}, {"name": "B"}],
                    "paging": {"start": 0, "count": 2, "total": 10}
...
def test_restliclient(
    ...
    http_method = request_override_method.lower() if request_override_method is not None else RESTLI_METHOD_TO_HTTP_METHOD_MAP[restli_method.value.upper()].lower()
    ...
    request_matchers = [matchers.header_matcher({k: v}) for (
        k, v) in checked_headers_dict.items()] if checked_headers_dict else []
    ...
    if request_body:
        ...
        if request_body_content_type == "urlencoded":
            request_matchers.append(matchers.urlencoded_params_matcher(request_body_value))
